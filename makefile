...
$(BIN)/ut_all: $(TEST)/ut_midterm2.cpp $(StaticLIB)
	g++ $(CFLAGS) $(Wfatal) -o $@ $< $(StaticLIB) $(LIB)
	$@ --gtest_catch_exceptions=1

obj/flattened_set_visitor.o: $(SRC)/flattened_set_visitor.cpp $(SRC)/visitor.h
	g++ $(CFLAGS) $(Wfatal) -c $< -o $@

obj/evaluate_visitor.o: $(SRC)/evaluate_visitor.cpp $(SRC)/visitor.h
	g++ $(CFLAGS) $(Wfatal) -c $< -o $@
